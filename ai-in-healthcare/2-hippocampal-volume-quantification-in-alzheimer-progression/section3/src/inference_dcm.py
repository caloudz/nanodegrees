...
def img_flip(slice):
    return np.flip((slice/np.max(slice))*0xff).T.astype(np.uint8)

def img_transpose(nd_img_pred):
    mask = nd_img_pred > 0
    return nd_img_pred[mask]

def img_convert_pil(nd_img):
    size = (200,200)
    return Image.fromarray(nd_img, mode="L").convert("RGBA").resize(size)

def img_get_pil(slice, slice_pred):
    # Numpy array needs to flipped, transposed and normalized to a matrix of values in the range of [0..255]
    # flip
    nd_img, nd_img_pred = img_flip(slice), img_flip(slice_pred)
    # transpose
    mask = nd_img_pred > 0
    nd_img[mask] = img_transpose(nd_img_pred)
    # create PIL from numpy array
    return img_convert_pil(nd_img)
...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    draw.multiline_text(
        ...
        f"Overlay masks over original saggital, coronal, and axial slices:\n",
        (255, 255, 255), font=main_font)

    # show original with overlay mask
    # get slices from original vol
    sag_slice = orig_vol[slice_nums[0], :, :]
    cor_slice = orig_vol[:, slice_nums[1], :]
    axi_slice = orig_vol[:, :, slice_nums[2]]
    # get prediction masks 
    sag_slice_pred = pred_vol[slice_nums[0], :, :]
    cor_slice_pred = pred_vol[:, slice_nums[1], :]
    axi_slice_pred = pred_vol[:, :, slice_nums[2]]
    # resize to original volume size
    sag_slice_pred = sag_slice_pred[:sag_slice.shape[0],:sag_slice.shape[1]]
    cor_slice_pred = cor_slice_pred[:cor_slice.shape[0],:cor_slice.shape[1]]
    axi_slice_pred = axi_slice_pred[:axi_slice.shape[0],:axi_slice.shape[1]]
        
    # Create a PIL image from array
    pil_sag = img_get_pil(sag_slice, sag_slice_pred)
    pil_cor = img_get_pil(cor_slice, cor_slice_pred)
    pil_axi = img_get_pil(axi_slice, axi_slice_pred)
        
    # Paste the PIL image into our main report image object (pimg)
    size = (200,200)
    pimg.paste(pil_sag, box=(10, 400))
    pimg.paste(pil_cor, box=(20+size[0], 400))
    pimg.paste(pil_axi, box=(30+2*size[0], 400))
...
def get_series_for_inference(path):
    ...
    # certain way. Hint: inspect the metadata of HippoCrop series
...
if __name__ == "__main__":
    ...
    os_command("storescu 127.0.0.1 4242 -v -aec REPORT /home/workspace/out/report.dcm")
